...
- **Layout**: `src/` layout (`src/filterax/`)
- **Testing**: pytest
...
**All four checks must pass before any commit.** CI runs them on the entire repo (`ruff check .`), not just `src/filterax/`, so always run the commands below from the repo root.
...
uv run --group typecheck ty check src/filterax
```

> **Common pitfall**: Running `ruff check src/filterax/` instead of `ruff check .` misses import-sorting errors in `tests/` and `scripts/`. The CI workflow runs `ruff check .`. Always use `.` (repo root), not a subdirectory.

## Key Directories

| Path | Purpose |
|------|---------|
| `src/filterax/` | Main package source code |
| `tests/` | Test suite |
