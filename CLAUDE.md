...
make typecheck            # Type check: ty check src/filterax
...
uv run --group lint ruff check .              # Lint — ENTIRE repo, not just src/filterax/
uv run --group lint ruff format --check .     # Format — ENTIRE repo
uv run --group typecheck ty check src/filterax  # Typecheck — package only
```

**Critical**: Always lint/format with `.` (repo root), not `src/filterax/`. CI runs `ruff check .` which includes `tests/` and `scripts/`.

## Architecture

### Package structure

All implementation lives in `src/filterax/`. The public API is re-exported through `src/filterax/__init__.py`.
